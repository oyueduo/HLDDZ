...
void GameScene::restartGame()
{
	...
	//当前出牌类型
	 ...
	 bujiaoTimes =0;
	 this->runAction(Sequence::create(DelayTime::create(2.0f),CallFunc::create([&]() {init(); }),NULL));
	 /*init();*/
}
		...
		/*for (auto i = 0; i < _currentPlayer->getCurrentCard().size(); i++)
			...
			if (_currentPlayer == _player1)
			{
				...
				else   //牌不合理
				{
					...
					sp1->runAction(Sequence::create(FadeOut::create(13.0f), RemoveSelf::create(), NULL));
					sp2->runAction(FadeOut::create(12.0f));
					sp3->runAction(FadeOut::create(12.0f));
					return;
				}
			}
...
void GameScene::updateClock(Player * player, int time, float delayTime)
{
	...
	_clock->runAction(Sequence::create(DelayTime::create(delayTime),
		FadeOut::create(0.1f),
		CallFunc::create([=]() {
		if (player == _player2)
		{
			_clock->setPosition(Vec2(PLAYER2X, PLAYER2Y));
		}
		else if (player == _player3)
		{
			_clock->setPosition(Vec2(PLAYER3X, PLAYER3Y));
		}
		else if (_player1->getLandlord() == false && _player2->getLandlord() == false && _player3->getLandlord() == false)
		{
			_clock->setPosition(Vec2(PLAYER1X, PLAYER1Y));
		}
		else
		{
			_clock->setPosition(Vec2(PLAYER1X2, PLAYER1Y2));
		}
		_clock->setTime(time);
	}),
		FadeIn::create(0.1f),
